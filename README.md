沒問題！這是一份完全符合您要求的**中文版期末專題報告草稿**。

這份文件重點突出了您強調的兩個核心功能：

1.  **自動堆疊模式**（程式碼寫好，手臂自己跑）。
2.  **手動測試模式**（可以控制前後左右，用來校正）。

請將以下內容直接複製到您的 GitHub `README.md` 或 HackMD 中，括號 `[ ]` 的地方請記得換成您實際的照片或連結。
...
本專題使用以下硬體組件：
  ...
  * **控制器:** Raspberry Pi (3B+/4B/5 皆可) [圖片: Raspberry Pi]
  * **機械手臂:** MeArm 壓克力套件 (v0.4)
  * **致動器:** SG90 伺服馬達 x 4 (底座、左臂、右臂、夾爪)
  * **電源供應 (關鍵):**
      * 樹莓派：使用原廠 5V/3A 電源。
      * 馬達：**獨立供電** (使用 4顆 AA 電池盒或 5V/3A 變壓器)。**注意：所有 GND 必須共地。**
...
[在此插入您的電路連接照片或 Fritzing 圖]
*(建議：放一張麵包板接線的特寫，證明有做電源隔離)*
...
4.  **成果展示：** 手臂將依照預寫好的程式碼，自動將積木搬運並堆疊成形。
